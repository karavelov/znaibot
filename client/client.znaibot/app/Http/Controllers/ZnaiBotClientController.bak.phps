...
class ZnaiBotClientController extends Controller
{
    ...
    private function mqttPublish(string $topic, string $payload): void
    {
        ...
        if (!empty($username)) {
            $connectPayload .= $this->mqttEncodeString((string) $username);
        }
        if (!empty($password)) {
            $connectPayload .= $this->mqttEncodeString((string) $password);
        }
        ...
        $ackPacketId = unpack('n', substr($pubAck, 2, 2))[1] ?? 0;
        if ($ackPacketId !== $packetId) {
            fclose($socket);
            throw new \RuntimeException('MQTT publish ack packet mismatch');
        }
    ...
    private function generateAiAnswer(string $userMessage): array
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                return [
                    'answer' => $answer,
                    'tts_url' => $this->buildTtsUrl($answer),
                    'ok' => true,
                ];
            }
            ...
            return ['answer' => 'Няма връзка с AI сървъра. Проверете дали Python скриптът работи.', 'ok' => false];
        }
    }
   ...
   public function searchTeacherLocation(Request $request)
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (preg_match('/([1-4][0-9]{2})/', $answer, $matches)) {
                    $room = $matches[1];
                    if (isset($this->schoolMapData[$room])) {
                        $location = $this->schoolMapData[$room];
                        $location['room'] = $room;
                    }
                }
                return response()->json([
                    ...
                    'answer' => $answer,
                    'location' => $location,
                    'tts_url' => route('tts.proxy', ['text' => $answer])
                ]);
            }
            return response()->json(['ok' => false, 'answer' => 'Грешка от AI сървъра.']);
        } catch (\Throwable $e) {
            return response()->json(['ok' => false, 'answer' => 'Грешка при връзка: ' . $e->getMessage()]);
        }
    }
    ...
    {
        return view('student_games', ['user' => $this->getScannedUser()]);
    }
    ...
    public function startRemoteStt(Request $request)
    {
        $validated = $request->validate([
            'mode' => 'nullable|string|in:dictation,conversation',
        ]);
        $sessionId = (string) Str::uuid();
        $mode = (string) ($validated['mode'] ?? 'dictation');
        $topic = env('MQTT_STT_TOPIC', 'znaibot/stt/listen');
        $this->sttStorePut($sessionId, [
            'status' => 'pending',
            'session_id' => $sessionId,
            'mode' => $mode,
        ]);
        $callbackToken = env('STT_SHARED_TOKEN', 'change-me');
        $callbackUrl = rtrim((string) $request->getSchemeAndHttpHost(), '/') . '/api/stt/ingest';
        try {
            $payload = [
                'session_id' => $sessionId,
            ...
            $this->mqttPublish($topic, json_encode($payload, JSON_UNESCAPED_UNICODE));
            Log::info('STT start published', [
                'session_id' => $sessionId,
                'mode' => $mode,
                'topic' => $topic,
                'callback_url' => $callbackUrl,
            ...
            Cache::forget("stt_result_{$sessionId}");
            return response()->json([
                'ok' => false,
                'message' => 'STT MQTT publish failed: ' . $e->getMessage(),
            ], 502);
        }
        return response()->json([
            'ok' => true,
    ...
    {
        $sessionId = trim((string) $request->input('session_id', $request->input('sessionId', '')));
        $rawTranscript = $request->input('text', $request->input('transcript', $request->input('transcription', '')));
        $transcript = trim((string) $rawTranscript);
        ...
        if (mb_strlen($sessionId) > 120 || mb_strlen($transcript) > 3000) {
            return response()->json([
                'ok' => false,
        ...
        $mode = in_array($requestedMode, ['dictation', 'conversation'], true)
            ? $requestedMode
            : (string) ($existing['mode'] ?? 'dictation');
        ...
        if ($mode === 'conversation') {
            $result = $this->generateAiAnswer($transcript);
        }
        ...
        $payload = [
            ...
            'tts_url' => $result['tts_url'] ?? null,
            'ok' => $result['ok'] ?? false,
        ];

        $this->sttStorePut($sessionId, $payload);
        Log::info('STT ingest stored', [
            'session_id' => $sessionId,
            'mode' => $mode,
            'text_len' => mb_strlen($transcript),
            'ok' => $payload['ok'],
        ]);
    ...
    public function getSttResult(string $sessionId)
    {
        ...
        if (!$payload) {
            Log::info('STT result expired', ['session_id' => $sessionId]);
            return response()->json([
                'status' => 'expired',
                'session_id' => $sessionId,
            ], 404)
        ...
        Log::info('STT result fetched', [
            'session_id' => $sessionId,
            'status' => $payload['status'] ?? null,
            'mode' => $payload['mode'] ?? null,
            'text_len' => mb_strlen((string)($payload['transcript'] ?? $payload['text'] ?? '')),
        ]);
    ...
    public function getSttResultApi(string $sessionId)
    {
        return $this->getSttResult($sessionId);
    }
    ...
    public function proxyTts(Request $request)
    {
        ...
        $topic = env('MQTT_TTS_TOPIC', 'znaibot/tts/play');
        $token = env('TTS_TOKEN', env('STT_SHARED_TOKEN', ''));
        $voice = (string) ($validated['voice'] ?? env('TTS_VOICE', 'bg-BG-BorislavNeural'));
        if (!$topic) {
            return response()->json(['ok' => false, 'message' => 'TTS MQTT topic is not configured'], 500);
        }
        $payload = [
        ...
        try {
            ...
            Log::error('TTS proxy failed', ['error' => $e->getMessage()]);
            return response()->json([
                'ok' => false,
                'message' => 'TTS MQTT publish failed: ' . $e->getMessage(),
            ], 502);
        }
    }
}