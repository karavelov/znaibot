...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">Нов предмет</h1>
            <p class="text-gray-400 dark:text-gray-500 text-sm font-medium mt-1">Добавяне на наименование на предмет</p>
        </div>
        <a href="{{ route('admin.subject-names.index') }}"
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-sm p-8 max-w-lg">
        <form action="{{ route('admin.subject-names.store') }}" method="POST">
            @csrf

            <div class="space-y-5">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">
                        Наименование <span class="text-red-400">(*)</span>
                    </label>
                    <input type="text"
                           name="name"
                           value="{{ old('name') }}"
