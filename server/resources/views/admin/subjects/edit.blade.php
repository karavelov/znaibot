...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-400 dark:text-gray-500 text-sm font-medium mt-1">{{ $assignment->subject->name }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-sm p-8 max-w-xl">
        <form action="{{ route('admin.subjects.update', $assignment->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <select name="subject_name"
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->name }}"
                                {{ old('subject_name', $assignment->subject->name) == $subject->name ? 'selected' : '' }}>
                                {{ $subject->name }}
                            </option>
                        @endforeach
                    </select>
