...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">Потребители</h1>
            <p class="text-gray-400 dark:text-gray-500 text-sm font-medium mt-1">Управление на всички потребители</p>
        </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#customerlist-table').on('preXhr.dt', function(e, settings, data) {
                data.role_filter = $('#role-filter').val() || '';
            });

            $(document).on('change', '#role-filter', function() {
                $('#customerlist-table').DataTable().draw();
            });
