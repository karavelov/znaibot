...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">Подкатегории</h1>
            <p class="text-gray-400 dark:text-gray-500 text-sm font-medium mt-1">Управление на подкатегориите за публикации</p>
        </div>
        <a href="{{ route('admin.blog-sub-category.create') }}"
    ...
    <script>
        $(document).ready(function(){
            $('body').on('click', '.change-status', function(){
                ...
                $.ajax({
                    url: "{{route('admin.blog-sub-category.status-change')}}",
                    method: 'PUT',
                    data: {
                        status: isChecked,
                        id: id
