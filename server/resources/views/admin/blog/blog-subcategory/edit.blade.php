...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">Редактиране на подкатегория</h1>
            <p class="text-gray-400 dark:text-gray-500 text-sm font-medium mt-1">{{ $blogSubCategory->name }}</p>
        </div>
        <a href="{{ route('admin.blog-sub-category.index') }}"
    ...
    <!-- Form Card -->
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-sm p-8 max-w-lg">
        <form action="{{ route('admin.blog-sub-category.update', $blogSubCategory->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Главна категория</label>
                    <select name="blog_category_id"
                        ...
                        @foreach ($blogCategories as $category)
                            <option {{ $category->id == $blogSubCategory->blog_category_id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $blogSubCategory->name }}"
                ...
                <div>
                    ...
                    <select name="status"
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl text-sm font-medium text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all">
                        <option {{ $blogSubCategory->status == 1 ? 'selected' : '' }} value="1">Публично</option>
                        <option {{ $blogSubCategory->status == 0 ? 'selected' : '' }} value="0">Скрито</option>
                    </select>
                </div>
