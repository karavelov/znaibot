...
<div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
            <span class="w-5 h-5 rounded-full shadow-sm flex-shrink-0" style="background-color: {{ $allergen->color }};"></span>
            <div>
                <h1 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">{{ $allergen->name }}</h1>
                <p class="text-gray-400 text-sm font-medium mt-0.5">Редакция на алерген</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-xl">
        <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-sm p-8">
            <form action="{{ route('admin.allergens.update', $allergen->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="color"
                               value="{{ old('color', $allergen->color) }}"
